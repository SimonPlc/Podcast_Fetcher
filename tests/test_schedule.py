...
def test_slot_is_todays_date_once_the_scheduled_time_has_passed() -> None:
    # 2026-08-26 is a Wednesday: a digest day, slot at 22:47 UTC.
    assert most_recent_due_slot(utc(2026, 8, 26, 22, 48)) == date(2026, 8, 26)


def test_slot_is_the_previous_day_before_todays_scheduled_time() -> None:
    assert most_recent_due_slot(utc(2026, 8, 26, 22, 40)) == date(2026, 8, 25)
...
def test_friday_and_saturday_have_no_slot_of_their_own() -> None:
    ...
    # Sunday, before 22:47: still Thursday's, not a phantom weekend slot.
    assert most_recent_due_slot(utc(2026, 8, 30, 10, 0)) == thursday
    # Sunday, after 22:47: the new week's first slot.
...
def test_grace_clears_the_backup_cron() -> None:
    """The backup fires 54 minutes after the primary and needs a few more
    to install deps and send. The default grace must not expire first, or
    a merely-slow backup would be pre-empted by a catch-up brief."""
    backup_still_working = utc(2026, 8, 26, 23, 50)
